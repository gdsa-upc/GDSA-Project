...
def get_features(params,pca=None,scaler=None):
    ...
    km1 = pickle.load (open(os.path.join(params['root'],params['root_save'],
                                     params['codebooks_dir'],'codebook_'
                                     + str(params['descriptor_size']) + "_"
                                     + "ORB"
                                     + "_" + "ORB" + '.cb'),'rb'))
    ...
    for image_name in image_list:
        ...
        feats1 = image_local_features(im)
        ...
        if feats and feats1 is not None:
            ...
            if scaler is not None:
                ...
                scaler.transform (feats1)
            # Whiten if needed
            if pca is not None:

                pca.transform(feats)
                pca.transform(feats1)

            # Compute assignemnts
            assignments, assignments1 = get_assignments(km, km1, feats,feats1)

            # Generate bow vector
            feats = bow(assignments,assignments1,km,km1)
        else:
            # Empty features
...
def train_codebook(params,X,Y):

        # Init kmeans instance
        km = MiniBatchKMeans(params['descriptor_size'])
        km1 = MiniBatchKMeans(params['descriptor_size'])
    ...
    # normalize dataset for easier parameter selection
        ...
        pickle.dump(km1, open(os.path.join(params['root'],params['root_save'],
                                     params['codebooks_dir'],'codebook_'
                                     + str(params['descriptor_size']) + "_"
                                     + "ORB"
                                     + "_" + "ORB" + '.cb'),'wb'))
...
def get_assignments(km, km1,descriptors, descriptors1):

    assignments = km.predict(descriptors)
    assignments1 = km1.predict(descriptors1)

    return assignments, assignments1


def bow(assignments, assignments1,km, km1):

    # Initialize empty descriptor of the same length as the number of clusters
    descriptor2 = np.zeros(np.shape(km.cluster_centers_)[0])
    descriptor1 = np.zeros(np.shape(km1.cluster_centers_)[0])
    # Build vector of repetitions
    for a in assignments:

        descriptor2[a] += 1
    for b in assignments1:
        descriptor1[b] +=1
    # L2 normalize
    descriptor2 = normalize(descriptor)
    descriptor1 = normalize(descriptor1)
    descriptor= np.concatenate(descriptor2, descriptor1)
