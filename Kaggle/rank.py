...
def rank(params):
    ...
    for test_id in test_features.keys():
        ...
        # Save to text file
        outfile = open(os.path.join(params['root'],params['root_save'],params['rankings_dir'],params['descriptor_type'],params['split'], test_id.split('.')[0] + '.txt'),'w')
        #outfile = open(os.path.join(params['root'],params['root_save'],params['rankings_dir'],params['descriptor_type'],,val_id.split('.')[0] + '.txt'),'w')
        for item in ranking:
